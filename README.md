...
- 在备份页配置用户自己的 Supabase 项目后，使用 Supabase Auth 与 RLS 保存加密的 `profiles.json` 副本。备份密码经 Argon2id 派生 AES-GCM 密钥；登录密码、访问令牌和 Supabase secret/service key 不会保存或使用。
- 展示诊断信息和恢复历史。
...
- Windows 系统托盘常驻：左键恢复主窗口；右键原生菜单只读显示 Codex / Claude 当前路由并提供显式退出。
...
| 本地数据 | 应用数据目录中的 `state/profiles.json`、`state/settings.json`、`state/cloud-backup.json`、备份元数据与受限的应用日志目录 |
...
- `npm run dev`：启动真实 `Tauri` 后端并打开 `http://localhost:1420` 的浏览器页面。浏览器经 Vite 代理调用这个本机进程的同一套类型化命令，读取、预览和确认后的写入都作用于实际本机状态与客户端配置。
