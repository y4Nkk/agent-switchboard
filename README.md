...
- **Codex 官方订阅额度**：官方登录档案可只读显示账户的限额窗口百分比与重置时间；查询直接使用现有 Codex OAuth 登录态，结果不含令牌且不会改写登录缓存。
- **供应商档案**：每个 Codex / Claude Code 档案是独立文件，可新建、编辑、排序、发现导入、从 CC Switch 导入或删除。
- **通用设置与官方目录**：基础参数只管理可安全投影的用户级客户端偏好，并支持保存、恢复默认值与只读片段预览；官方设置目录同时列出全局指令、供应商专属模块以及项目/本地/受管/登录等保留边界，不把它们误作可写开关。
...
- **本机辅助工作流**：只读发现、全局指令管理、备份与恢复、受限运行日志、端点探测、会话查看与恢复命令。
...
| 官方登录缓存 | Codex 或 Claude Code | 不读取为档案、不复制、不删除、不接管；Codex 官方订阅额度仅临时读取现有 OAuth 身份以查询服务端窗口，不持久化凭据 |
