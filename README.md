...
- **Codex 官方订阅额度**：官方登录档案只读显示限额窗口用量百分比、重置时间和真实读取的本地趋势。
- **系统托盘**：按客户端分组快速切换，选择仍走同一预览与事务流程。
- **会话**：只读浏览本机 Codex / Claude Code 会话记录，一键在新终端恢复。
- **用量与探测**：供应商可配置用量余额查询（声明式或受限脚本）与端点连通检测；独立的「用量」页按本机日历日汇总 Codex / Claude Code 会话中的模型 token，展示趋势、构成与明细，但不将其视为供应商剩余额度。供应商余额与 Codex 官方额度在成功读取后各自写入本地、无凭据的历史账本，绝不伪造趋势或混合单位。
...
- 密钥保存在应用数据目录，并在确认切换时写入客户端要求的受管位置：Codex 使用官方 `auth.json` 的 API-key 缓存形态，Claude 使用 `env.ANTHROPIC_AUTH_TOKEN`；预览、差异、日志与错误一律脱敏。
- 本地运行：不是模型网关，不代理或转发请求；没有账号体系与遥测；Codex 自定义切换仅通过事务读写 API-key 登录字段并保留 OAuth token，官方登录仍由专用流程处理；不使用 CDP 注入、安装包补丁或改写会话 / SQLite；未经确认不创建或替换任何真实客户端配置文件。
...
每次推送都会运行三端打包工作流：四个 job（Windows NSIS、macOS 双架构 dmg、Linux deb + AppImage）各自执行 Rust 与前端测试、构建安装包并保留为 GitHub Actions 制品 30 天；所有产物在发布前逐个执行凭据扫描。推送与根 `Cargo.toml` 版本严格一致的 `v*` 标签时，工作流额外生成 Minisign 签名的 Windows NSIS、macOS `.app.tar.gz`、Linux deb 与 AppImage 更新载荷；只有全部五种更新目标完成、签名和扫描均通过时才创建稳定 GitHub Release。安装完包含此更新器的版本后，应用会检查经签名的 `latest.json`，验证载荷、下载并安装；Windows NSIS 自动重启，macOS / Linux 在安装完成后由应用重启。`latest.json` 始终最后上传，因此客户端不会发现不完整的发布。CNB 同时通过根目录 `.cnb.yml` 在 Linux Runner 上自动执行 Rust 与前端验证。
