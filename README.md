...
- 概览本机会话速览：用户显式读取每个客户端已发现的会话数量、最近本地记录更新时间与扫描提示，并可进入会话管理；不显示会话标题、路径或对话内容。
- Windows 系统托盘常驻：左键恢复主窗口；右键原生菜单只读显示 Codex / Claude 当前路由并提供显式退出。
- 顶层「设置」页管理应用自身的窗口行为与外观：关闭策略、主题、动态效果、始终置顶；不写入任何客户端配置文件。
...
| 本地数据 | 应用数据目录中的 `state/profiles.json`、`state/settings.json` 与备份元数据 |
| 凭据 | 档案 `apiKey` 明文保存于应用数据目录；切换将其分别写入 Codex bearer token 或 Claude auth token，所有展示/诊断均脱敏 |
