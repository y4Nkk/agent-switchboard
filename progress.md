...
- 备份浏览器、恢复流程和发布打包。
- 在通用设置中直接管理 Codex 全局 `AGENTS.md` 与 Claude Code 全局 `CLAUDE.md`，不建立第二份提示词存储。
...
**供应商卡连通性测试图标（完成，2026-08-31）：** 供应商行右侧既有动作簇新增无文字连通性图标。点击直接复用 `probe_endpoint` 对该档案的服务地址做检测，检测中禁用图标，结果（结论 / HTTP 状态 / 延迟 / 时间与范围说明）在同一张卡片内展开；不新增第二个网络 API、凭据路径或模型请求。编辑器与卡片共享 `useEndpointProbe` 请求生命周期和 `ProbeFeedback` 展示，地址变更时两处均废弃旧结果。验证：`npx vitest run src/components/ProbePanel.test.tsx src/components/ProviderList.test.tsx`（21 项）、本轮组件依赖图 `tsc`、`npx vite build --logLevel error` 与目标文件 `git diff --check` 通过；全仓 `npm run typecheck` 仍被并行测试夹具缺少 `runtimeLogLevel` 字段阻断，未改动该无关契约。

**路由与模型映射（2026-08-26 第二轮，2026-08-31 收敛）：** 供应商档案均表示自定义服务，空地址不得保存。Codex 档案固定写回 `model_provider = "openai"`，并通过官方 `openai_base_url` 使用自定义服务；不创建或覆盖任何 `model_providers.*` 表。Claude 官方登录不作为档案管理。Claude 档案使用主模型、Haiku、Sonnet、Opus 与 `availableModels`；已弃用的 `ANTHROPIC_SMALL_FAST_MODEL` 会在下一次切换清理，导入时升格为 Haiku 档。Codex 的运行参数属于供应商档案，通用配置只保留真正的全局键（Codex `disable_response_storage`）。旧 `profiles.json` 迁移会将可确定的旧档案级通用设置写入同客户端的每个档案；遇到凭据引用、无法归属的服务表或无效形状时停止迁移且不改写原存储，绝不静默丢弃数据。
...
**软件日志 Tab（2026-08-31 完成）：** 用户澄清要的是软件自身日志后，撤回本轮误接的 `SwitchLog` 审计列表路径与主从详情界面；`SwitchLog` 本身仍只服务既有匹配状态和撤回。新唯一后端所有者为 `src-tauri/src/runtime_log.rs`：官方 `tauri-plugin-log` 仅接受本应用的闭合 JSON 运行事件，当前文件 20 MiB、保留 4 个轮转归档、跨启动追加；记录应用启动及显式的应用设置/档案、通用设置、配置切换与恢复、锁恢复、云端备份、会话恢复、CC Switch 档案导入操作，成功写 `info`，失败只写既有稳定错误码，绝不写档案名、路径、配置、密钥、请求/响应、异常消息或栈。`list_runtime_logs` 同时经 Tauri 与浏览器开发桥只读该应用日志目录，解析当前及归档文件、倒序返回最多 500 条；日志页自己加载/刷新，不再混入 `useConfigSnapshot` 的配置状态轮询，提供「全部 / 信息 / 错误」筛选及时间、级别、事件、错误码表格。新增 `open_runtime_log_dir`：后端自行解析并在必要时创建应用日志目录，再交给系统文件管理器，渲染层不接收路径；日志页提供对应的「打开日志文件夹」按钮。公开 CC Switch 源码只作为“统一后端记录 → 有界文件轮转 → 前端只读”的职责参考，未复制代码或视觉。验证：隔离构建目录中的 `cargo check -p agent-switchboard` 通过；`cargo test -p agent-switchboard runtime_log --lib` 3/3；`npm run typecheck` 通过；`npx vitest run src/pages/LogsPage.test.tsx src/api/client.test.ts` 17/17；`npx vitest run src/App.test.tsx src/boundary.test.ts` 27/27；`git diff --check` 通过。按用户要求未启动桌面/浏览器界面，也未读取真实配置。
