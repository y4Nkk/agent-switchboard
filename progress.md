...
**官方登录接入方式（2026-09-02 立场修订先行）：** 用户要求供应商「新建供应商」支持与 cc-switch 一致的官方登录。调研 cc-switch 源码确认其 OAuth 认证中心仅覆盖 Codex（OpenAI 设备码流程）/ GitHub Copilot / xAI，无 Claude OAuth；按用户确认，本项目扩展到双客户端：Codex 沿用 OpenAI 设备码（POST `auth.openai.com/api/accounts/deviceauth/usercode` 取 user_code → 用户在 `auth.openai.com/codex/device` 输码 → 轮询换 token），Claude 新增 OAuth PKCE + 本地回环（`claude.ai/oauth/authorize` → `127.0.0.1` 临时端口收回调 → 换 token）。该功能要求反转既有所有权立场：`auth.json` 与 `~/.claude/.credentials.json` 由「绝不接管」改为「仅在用户明确发起官方登录或重新登录时由专用登录流程写入，其余时间只读观测」，令牌不进入供应商档案、渲染层、日志或错误信息。已同步 AGENTS.md §5、README（定位声明、能力一览、所有权表、边界清单）与 `asb-core::ownership` 官方设置目录两条目文案。风险记录：Codex 设备码端点字段名取自 cc-switch 生产实现（serde 宽容解析，真实登录为验收门）；Claude 订阅类型采用社区验证写法（`claude /status` 验收）；OAuth client_id 为厂商公开标识（非机密），HTTP 客户端使用本项目自有 UA。退出条件：双客户端官方登录 + 官方条目重新登录全链路测试通过。
...
**标签发布保留已完成产物（2026-09-03 完成，用户指令）：** `package.yml` 的 `release` job 不再以整个矩阵成功为前提：标签运行等待所有矩阵项结束后仍会运行，并下载每个已成功上传、已通过凭据扫描的安装包。至少存在一个包时创建正式 GitHub Release；没有任何包时以明确错误拒绝创建空 Release。重跑同一标签时改用 `gh release upload --clobber` 覆盖同名 asset，不再依赖不受 `gh release create` 支持的 `--clobber` 参数。Release 说明明确其只列出可发布产物，因此一个平台失败不会阻断其他平台的安装包显示。
