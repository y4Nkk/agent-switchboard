...
**1M 勾选内联到模型行（完成，2026-09-03）：** 按用户指令把三处 `1M` 启用勾选从模型下方独占一行改为内联在各自模型行右侧，避免换行显示：主模型的 `1M` 位于快捷下拉与「检测连通 / 获取模型」操作之间，`Claude` Sonnet / Opus 档的 `1M` 内联在各自输入框行内，`Codex` 的上下文窗口勾选同位显示并随之删除只含这一项的「模型运行参数」字段集。可见文案统一为紧凑「`1M`」，完整语义（如「主模型启用 1M 上下文」）经 `Checkbox` 新增的可选 `ariaLabel` 保留为无障碍名（原生 input `aria-label` 优先于包裹 label 文本），既有按名称定位的测试契约零改动。`base.css` 为行内 `1M` 勾选补充 `flex: none` + 垂直居中规则。同轮完成「无兼容无过渡无技术债」清理：`activeModel` 在 `ProviderEditor` / `PreviewInspector` 上由可选属性改为必选，同轮更新全部 26 处测试调用点。已验证：`npm run typecheck` 通过；`npx vitest run src/components/ProviderEditor.test.tsx src/components/Checkbox.test.tsx` 26 项通过；`ProvidersPage` + `App` 集成 56 项通过；未启动或打开浏览器。

**模型行布局修复与启用模型展示（完成，2026-09-03）：** 按用户指令完成两项修正。①主模型行布局：旧 `.asb-model-control` 栅格（`grid-row: 1` 的按钮在自动放置时会抢占 `1fr` 轨道）导致「检测连通」占满整行、主模型输入框与快捷下拉被挤到边缘，现改为弹性换行布局——输入框 `flex: 1 1 16rem` 弹性伸缩，快捷下拉与「检测连通」「获取模型」保持固有宽度，探测结果占满下方整行；720px 媒体查询里三条栅格覆盖规则随旧布局删除（换行行为由弹性布局天然承担）。②「当前启用模型」上下文（用户选定的语义：显示真实客户端配置中生效的模型）：唯一数据来源是应用快照的 `ConfigFileStatus.route.model`（`App.tsx` 按当前客户端过滤后经 `ProvidersPage` 下发，绝不重新推导或读草稿值）；供应商编辑器在主模型输入框下方以弱化说明行显示「当前启用模型：X」（无启用模型时不显示该行），变更预览面板顶部新增键值行显示同一事实（空值显示「—」），Codex / Claude 两个客户端共用同一实现。已验证：`npm run typecheck`、`npm run build` 通过；`npx vitest run` 编辑器 / 预览 / 快捷下拉 / 供应商页 / 列表 / `App` 集成六文件 87 项中除一项外全部通过（唯一失败为并行会话正在进行的用量刷新契约改动 `refreshIntervalMinutes` 引起的 `ProvidersPage` 用量工作区断言，非本轮引入；`App.test` 两处断言因预览新增「当前启用模型」行与差异 before 值同文本，已改为 `getAllByText` 并注明原因）；未启动或打开浏览器。
